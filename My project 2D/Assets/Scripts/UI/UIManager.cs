...
public class UIManager : MonoBehaviour
{
    ...
    private const string StartGameAfterLoadKey = "StartGameAfterLoad";
    ...
    private void Start()
    {
        ...
        SetUpUIElements();
        UpdateUI();
        if (PlayerPrefs.GetInt(StartGameAfterLoadKey, 0) == 1)
        {
            PlayerPrefs.DeleteKey(StartGameAfterLoadKey);
            StartGame();
        }
        else
        {
    ...
    /// <summary>
    /// Description:
    /// Starts gameplay from a menu page.
    /// Inputs:
    /// none
    /// Returns:
    /// void (no return)
    ...
    {
        if (ShouldReloadSceneForNewGame())
        {
            PlayerPrefs.SetInt(StartGameAfterLoadKey, 1);
            Time.timeScale = 1;
    ...
    private bool ShouldReloadSceneForNewGame()
    {
        return GameManager.instance != null &&
            (GameManager.instance.gameIsOver ||
            GameManager.instance.player == null ||
            !GameManager.instance.player.activeInHierarchy);
    }
