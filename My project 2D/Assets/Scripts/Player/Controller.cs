...
public class Controller : MonoBehaviour
{
    ...
    [Tooltip("If enabled, clamp the player's position so it cannot leave the rectangle defined below.")]
    public bool useBoundaries = true;
    [Tooltip("Minimum X / Y the player can reach.")]
    public Vector2 boundaryMin = new Vector2(-9f, -5f);
    [Tooltip("Maximum X / Y the player can reach.")]
    public Vector2 boundaryMax = new Vector2(9f, 5f);
    ...
    private void MovePlayer(Vector3 movement)
    {
        ...
        // Keep the player inside the configured rectangle so it cannot leave the playable area.
        if (useBoundaries)
        {
            Vector3 clamped = transform.position;
            clamped.x = Mathf.Clamp(clamped.x, boundaryMin.x, boundaryMax.x);
            clamped.y = Mathf.Clamp(clamped.y, boundaryMin.y, boundaryMax.y);
