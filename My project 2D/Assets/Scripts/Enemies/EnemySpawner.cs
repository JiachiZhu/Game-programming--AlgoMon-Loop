...
public class EnemySpawner : MonoBehaviour
{
    ...
    [Tooltip("Spawn delay will never go below this value, no matter how high the score gets.")]
    public float minSpawnDelay = 0.4f;
    [Tooltip("How much (in seconds) the spawn delay shrinks for every 100 points of score.")]
    public float delayReductionPer100Score = 0.3f;
    ...
    // Returns the spawn delay scaled down by the current player score.
    private float GetCurrentSpawnDelay()
    {
        int currentScore = (GameManager.instance != null) ? GameManager.score : 0;
        float reduction = (currentScore / 100f) * delayReductionPer100Score;
        return Mathf.Max(minSpawnDelay, spawnDelay - reduction);
    }
    ...
    private void CheckSpawnTimer()
    {
        ...
        if (Time.timeSinceLevelLoad > lastSpawnTime + GetCurrentSpawnDelay() && (currentlySpawned < maxSpawn || spawnInfinite))
        {
